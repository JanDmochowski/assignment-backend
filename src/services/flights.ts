...
import { Flight, SearchFlightsOptions } from '../modules/types';
...
export async function searchFlights(emptySearch: boolean, options: SearchFlightsOptions) {
  ...
  const resultUnoptimized = flights.filter((flight: Flight) => {
    if (
      ...
      ||
      (flight.origin !== airports[destination]?.name && flight.destination !== airports[destination]?.name)
    ) {
      return false;
    }
  ...
  if (!emptySearch) {
    if (service === 'amadeusBestPrice' && departureDate && returnDate) {
      ...
      const departures = resultUnoptimized.filter((flight) => flight.origin === airports[origin].name);
      const arrivals = resultUnoptimized.filter((flight) => flight.origin === airports[destination].name);

      departures.forEach((flight) => {
        arrivals.forEach((el) => {
          if (moment(el.departureDate).isSame(moment(flight.departureDate).add(travelDuration, 'days')) &&
            ...
            if (result.length) {
              result[0] = {
                ...flight,
                returnDate: moment(flight.departureDate).add(travelDuration, 'days').format('YYYY-MM-DD'),
                price: {
                ...
                uuid: uuid.v1(),
                seatAvailability: flight.seatAvailability < el.seatAvailability ? flight.seatAvailability : el.seatAvailability,
              };
            } else {
              result.push(
                {
